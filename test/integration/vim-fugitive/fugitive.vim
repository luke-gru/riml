...
function! s:_throw(string) abort
...
function! s:repo_superglob(base) dict abort
  if a:base =~# '^/' || a:base !~# ':'
    ...
    if a:base !~# '^/'
      ...
      let heads += sort(split(s:repo() . s:git_chomp("rev-parse", "--symbolic", "--branches", "--tags", "--remotes"), "\n"))
...
function! s:repo_config(conf) dict abort
  return matchstr(system(s:repo() . s:git_command('config') . ' ' . a:conf), "[^\r\n]*")
endfunction
function! s:repo_user() dict abort
  let username = s:repo() . s:config('user.name')
  let useremail = s:repo() . s:config('user.email')
...
function! s:buffer_sha1() dict abort
  if self.spec() =~# '^fugitive://' || self.spec() =~# '\.git/refs/\|\.git/.*HEAD$'
    return self.repo() . s:rev_parse(self.rev())
  else
    return ''
  endif
endfunction
...
function! s:GitComplete(A, L, P) abort
  ...
  if a:L =~# ' [[:alnum:]-]\+ '
    return s:repo() . s:superglob(a:A)
  elseif a:A ==# ''
    return sort(cmds + keys(s:repo() . s:aliases()))
  else
    return filter(sort(cmds + keys(s:repo() . s:aliases())), 'v:val[0:strlen(a:A)-1] ==# a:A')
  endif
endfunction
function! s:DirComplete(A, L, P) abort
  let matches = s:repo() . s:dirglob(a:A)
  return matches
endfunction
...
function! fugitive#reload_status() abort
  ...
  try
    ...
    unlet! s:reloading_statusendtry
endfunction
...
function! s:StagePrevious(count)
  if line('.') ==# 1 && exists(':CtrlP')
    return 'CtrlP ' . fnameescape(s:repo() . s:tree())
  else
    for i in range(a:count)
      call search('^#\t.*', 'Wbe')
    endfor
...
function! s:StageReloadSeek(target, lnum1, lnum2)
  ...
  silent! edit!
  1redrawcall search('^#\t\%([[:alpha:] ]\+: *\|.*\%uff1a *\)\=\V' . jump . '\%( ([^()[:digit:]]\+)\)\=\$', 'W')
endfunction
...
function! s:StagePatch(lnum1, lnum2) abort
  ...
  try
    ...
    if exists('first_filename')
      silent! edit!
      1redrawcall search('^#\t\%([[:alpha:] ]\+: *\)\=\V' . first_filename . '\%( ([^()[:digit:]]\+)\)\=\$', 'W')
    endif
  catch /^fugitive:/
    return 'echoerr v:errmsg'
  endtry
...
function! s:Commit(args) abort
  let cd = exists('*haslocaldir') && haslocaldir() ? 'lcd ' : 'cd '
  let dir = getcwd()
  let msgfile = s:repo() . s:dir('COMMIT_EDITMSG')
  let outfile = tempname()
  let errorfile = tempname()
  try
    try
      execute cd . s:fnameescape(s:repo() . s:tree())
      if &shell =~# 'cmd'
        let command = ''
      ...
      let command .= s:repo() . s:git_command('commit') . ' ' . a:args
      if &shell =~# 'csh'
    ...
    if !v:shell_error
      ...
      if error =~# '\<false''\=\.$'
        ...
        let b:fugitive_commit_arguments = a:args
...
function! s:Grep(cmd, bang, arg) abort
  ...
  try
    execute cd . '`=s:repo().tree()`'
    let &grepprg = s:repo() . s:git_command('--no-pager', 'grep', '-n')
    let &grepformat = '%f:%l:%m'
    execute a:cmd . '! ' . escape(matchstr(a:arg, '\v\C.{-}%($|[''" ]\@=\|)@='), '|')
    let list = a:cmd =~# '^l' ? getloclist(0) : getqflist()
    for entry in list
      if bufname(entry.bufnr) =~# ':'
        let entry.filename = s:repo() . s:translate(bufname(entry.bufnr))
        unlet! entry.bufnr
      elseif a:arg =~# '\%(^\| \)--cached\>'
        let entry.filename = s:repo() . s:translate(':0:' . bufname(entry.bufnr))
        unlet! entry.bufnr
      endif
    endfor
    if a:cmd =~# '^l'
...
function! s:Log(cmd, ...)
  ...
  try
    execute cd . '`=s:repo().tree()`'
    let &grepprg = escape(call(s:repo() . git_command, a:cmd, s:repo()), '%#')
    let &grepformat = '%f::%m'
    execute a:cmd
  finally
    let &grepformat = grepformat
...
function! s:Edit(cmd, bang, ...) abort
  ...
  if a:bang
    ...
    if a:cmd =~# 'read'
      let git = buffer.repo() . s:git_command()
      let last = line('$')
      ...
      let s:temp_files[temp] = buffer.repo() . s:dir()
      silent execute a:cmd.' '.temp
      if a:cmd =~# 'pedit'
        wincmd P
      endif
      let _echo = s:Edit('read', 1, args)
      silent write!
  ...
  elseif a:0
    ...
    function! s:fugitive#foldtext() abort
  ...
  try
    let file = buffer.repo() . s:translate(file)
  catch /^fugitive:/
    return 'echoerr v:errmsg'
  endtry
  if a:cmd ==# 'read'
...
function! s:EditComplete(A, L, P) abort
  return map(s:repo() . s:superglob(a:A), 'fnameescape(v:val)')
endfunction
...
function! s:Write(force, ...) abort
  ...
  elseif s:buffer() . type() ==# 'index'
    return 'Gcommit'
  elseif s:buffer() . s:path() ==# '' && getline(4) =~# '^+++ '
    let filename = getline(4)[6 : -1]
    setlocal buftype=
    silent write
    setlocal buftype=nowrite
    if matchstr(getline(2), 'index [[:xdigit:]]\+\.\.\zs[[:xdigit:]]\{7\}') ==# s:repo() . s:rev_parse(':0:' . filename)[0 : 6]
      let err = s:repo() . s:git_chomp('apply', '--cached', '--reverse', s:buffer() . s:spec())
    else
      let err = s:repo() . s:git_chomp('apply', '--cached', s:buffer() . s:spec())
    endif
    if err !=# ''
  ...
  let mytab = tabpagenr()
  let mybufnr = bufnr('')
  let path = a:0 ? join(a:000, ' ') : s:buffer() . s:path()
  if path =~# '^:\d\>'
  ...
  let file = s:repo() . s:translate(path)
  let treebufnr = 0
  ...
  if treebufnr ># 0 && treebufnr !=# bufnr('')
    ...
    execute 'write! ' . s:fnameescape(s:repo() . s:translate(path))
  endif
  if a:force
    let error = s:repo() . s:git_chomp_in_tree('add', '--force', file)
  else
    let error = s:repo() . s:git_chomp_in_tree('add', file)
  endif
  if v:shell_error
    let v:errmsg = 'fugitive: ' . error
    return 'echoerr v:errmsg'
  endif
  if s:buffer() . s:path() ==# path && s:buffer() . s:commit() =~# '^\d$'
    set nomodified
  endif
  let one = s:repo() . s:translate(':1:' . path)
  let two = s:repo() . s:translate(':2:' . path)
  let three = s:repo() . s:translate(':3:' . path)
  for nr in range(1, bufnr('$'))
  ...
  unlet! restorewinnrlet zero = s:repo() . s:translate(':0:' . path)
...
function! s:Diff(bang, ...)
  ...
  elseif (!a:0 || a:1 ==# ':') && s:buffer() . s:commit() =~# '^[0-1]\=$' && s:repo() . s:git_chomp_in_tree('ls-files', '--unmerged', '--', s:buffer() . s:path()) !=# ''
    let nr = bufnr('')
  ...
  elseif a:0
    ...
    if file !~# ':' && file !~# '^/' && s:repo() . s:git_chomp('cat-file', '-t', file) =~# '^\%(tag\|commit\)$'
      let file = file . s:buffer() . s:path(':')
    endif
  else
    let file = s:buffer() . s:path(s:buffer() . s:commit() ==# '' ? ':0:' : '/')
  endif
  try
    let spec = s:repo() . s:translate(file)
    let commit = matchstr(spec, '\C[^:/]//\zs\x\+')
    if s:buffer() . s:compare_age(commit) <# 0
...
function! s:Move(force, destination)
  ...
  if isdirectory(s:buffer() . s:spec())
    let discarded = s:buffer() . s:setvar('&swapfile', 0)
  endif
  let message = call(s:repo() . git_chomp_in_tree, ['mv'] + (a:force ? ['-f'] : []) + ['--', s:buffer() . s:path(), a:destination], s:repo())
  if v:shell_error
  ...
  if s:buffer() . s:commit() ==# ''
    if isdirectory(a:destination)
      return 'keepalt edit ' . s:fnameescape(a:destination)
    else
      return 'keepalt saveas! ' . s:fnameescape(a:destination)
    endif
  else
    return 'file ' . s:fnameescape(s:repo() . s:translate(':0:' . a:destination))
  endif
endfunction
...
function! s:Remove(force)
  ...
  let message = call(s:repo() . git_chomp_in_tree, cmd + ['--', s:buffer() . s:path()], s:repo())
  if v:shell_error
...
function! s:linechars(pattern)
  ...
  if exists('*synconcealed') && &conceallevel ># 1
    for col in range(1, chars)
      let chars -= s:synconcealed(line('.'), col)[0]
    endfor
  endif
  return chars
endfunction
function! s:Blame(bang, line1, line2, count, args) abort
  try
    ...
    if s:buffer() . s:commit() =~# '\D\|..'
      let cmd += [s:buffer() . s:commit()]
    else
      let cmd += ['--contents', '-']
    endif
    let basecmd = escape(call(s:repo() . git_command, cmd + ['--', s:buffer() . s:path()], s:repo()), '!')
    try
      let cd = exists('*haslocaldir') && haslocaldir() ? 'lcd ' : 'cd '
      if !s:repo() . s:bare()
        let dir = getcwd()
      ...
      if a:count
        ...
        let current = line('.')
        let s:temp_files[temp] = s:repo() . s:dir()
        execute 'keepalt leftabove vsplit ' . temp
...
function! s:BlameCommit(cmd) abort
  ...
  if path ==# ''
    let path = s:buffer(b:fugitive_blamed_bufnr) . s:path()
  endif
  execute a:cmd
...
function! s:BlameSyntax() abort
  ...
  execute 'syn match FugitiveblameLineNumber         " *\d\+)\@=" contained containedin=FugitiveblameAnnotation' . conceal
  execute 'syn match FugitiveblameOriginalFile       " \%(\f\+\D\@<=\|\D\@=\f\+\)\%(\%(\s\+\d\+\)\=\s\%((\|\s*\d\+)\)\)\@=" contained nextgroup=FugitiveblameOriginalLineNumber,FugitiveblameAnnotation skipwhite' . (arg =~# 'f' ? '' : conceal)
  execute 'syn match FugitiveblameOriginalLineNumber " *\d\+\%(\s(\)\@=" contained nextgroup=FugitiveblameAnnotation skipwhite' . (arg =~# 'n' ? '' : conceal)
  execute 'syn match FugitiveblameOriginalLineNumber " *\d\+\%(\s\+\d\+)\)\@=" contained nextgroup=FugitiveblameShort skipwhite' . (arg =~# 'n' ? '' : conceal)
...
function! s:Browse(bang, line1, count, ...) abort
  try
    ...
    let full = s:repo() . s:translate(expanded)
    let commit = ''
    if full =~# '^fugitive://'
      let commit = matchstr(full, '://.*//\zs\w\+')
      let path = matchstr(full, '://.*//\w\+\zs/.*')
      if commit =~# '..'
        let type = s:repo() . s:git_chomp('cat-file', '-t', commit . s:sub(path, '^/', ':'))
      else
        let type = 'blob'
      endif
      let path = path[1 : -1]
    elseif s:repo() . s:bare()
      let path = '.git/' . full[strlen(s:repo() . s:dir()) + 1 : -1]
      let type = ''
    else
    ...
    if path =~# '^\.git/.*HEAD' && filereadable(s:repo() . s:dir(path[5 : -1]))
      let body = readfile(s:repo() . s:dir(path[5 : -1]))[0]
      if body =~# '^\x\{40\}$'
        let commit = body
        let type = 'commit'
        let path = ''
    ...
    elseif s:path =~# '^\.git/refs/remotes/.'
      ...
      if filereadable(s:repo() . s:dir('refs/remotes/' . branch))
        let remote = matchstr(branch, '[^/]\+')
        let rev = rev[strlen(remote) + 1 : -1]
      else
        if branch ==# ''
          let branch = matchstr(s:repo() . s:head_ref(), '\<refs/heads/\zs.*')
        endif
        if branch !=# ''
    ...
    let raw = s:repo() . s:git_chomp('config', 'remote.' . remote . '.url')
    if raw ==# ''
      let raw = remote
    endif
...
function! s:github_url(repo, url, rev, commit, path, type, line1, line2) abort
  ...
  if index(domains, 'http://' . matchstr(a:repo, '^[^:/]*')) >=# 0
    let root = 'http://' . s:sub(a:repo, ':', '/')
  else
    let root = 'https://' . s:sub(a:repo, ':', '/')
  endif
  if a:path =~# '^\.git/refs/heads/'
    let branch = a:repo.git_chomp('config', 'branch.' . a:path[16 : -1] . '.merge')[11 : -1]
    if branch ==# ''
      return root . '/commits/' . a:path[16 : -1]
    else
      return root . '/commits/' . branch
    endif
  elseif s:path =~# '^\.git/refs/.'
    return root . '/commits/' . matchstr(a:path, '[^/]\+$')
...
function! s:ReplaceCmd(cmd, ...) abort
  ...
  try
    ...
    silent execute 'keepalt file ' . s:fnameescape(fn)
    call delete(tmp)
    if fnamemodify(bufname('$'), ':p') ==# tmp
      silent execute 'bwipeout '.bufnr('$')
    endif
    silent execute 'doau BufReadPost ' . s:fnameescape(fn)
  endtry
endfunction
function! s:BufReadIndex()
  ...
  try
    ...
    if b:fugitive_display_format
      call s:ReplaceCmd(s:repo() . s:git_command('ls-files', '--stage'), index)
      set ft=git nospell
    else
      let cd = exists('*haslocaldir') && haslocaldir() ? 'lcd ' : 'cd '
      let dir = getcwd()
      try
        execute cd . '`=s:repo().tree()`'
        call s:ReplaceCmd(s:repo() . s:git_command('status'), index)
      finally
        execute cd . '`=dir`'
      endtry
      set ft=gitcommit
...
function! s:BufReadIndexFile()
  try
    ...
    let b:git_dir = s:repo() . s:dir()
    try
      call s:ReplaceCmd(s:repo() . s:git_command('cat-file', 'blob', s:buffer() . s:sha1()))
    finally
      if &bufhidden ==# ''
        setlocal bufhidden=delete
      endif
    ...
    silent execute 'doau BufNewFile ' . s:fnameescape(expand('%:p'))
    return ''
  catch /^fugitive:/
    return 'echoerr v:errmsg'
  endtry
...
function! s:BufWriteIndexFile()
  ...
  try
    ...
    let old_mode = matchstr(s:repo() . s:git_chomp('ls-files', '--stage', path), '^\d\+')
    if old_mode ==# ''
      let old_mode = executable(s:repo() . s:tree(path)) ? '100755' : '100644'
    endif
    let info = old_mode . ' ' . sha1 . ' ' . stage . "\t" . path
    call writefile([info], tmp)
    if has('win32')
      let error = system('type ' . s:gsub(tmp, '/', '\\') . '|' . s:repo() . s:git_command('update-index', '--index-info'))
    else
      let error = system(s:repo() . s:git_command('update-index', '--index-info') . ' < ' . tmp)
    endif
    if v:shell_error ==# 0
      setlocal nomodified
...
function! s:BufReadObject()
  try
    setlocal noro ma
    let b:git_dir = s:repo() . s:dir()
    let hash = s:buffer() . s:sha1()
    if !exists("b:fugitive_type")
      let b:fugitive_type = s:repo() . s:git_chomp('cat-file', '-t', hash)
    endif
    ...
    try
      if b:fugitive_type ==# 'tree'
        let b:fugitive_display_format = b:fugitive_display_format % 2
        if b:fugitive_display_format
          call s:ReplaceCmd(s:repo() . s:git_command('ls-tree', hash))
        else
          call s:ReplaceCmd(s:repo() . s:git_command('show', '--no-color', hash))
        endif
      elseif b:fugitive_type ==# 'tag'
        let b:fugitive_display_format = b:fugitive_display_format % 2
        if b:fugitive_display_format
          call s:ReplaceCmd(s:repo() . s:git_command('cat-file', b:fugitive_type, hash))
        else
          call s:ReplaceCmd(s:repo() . s:git_command('cat-file', '-p', hash))
        endif
      elseif b:fugitive_type ==# 'commit'
        ...
        if b:fugitive_display_format
          ...
          call s:ReplaceCmd(s:repo() . s:git_command('show', '--no-color', '--pretty=format:tree %T%nparent %P%nauthor %an <%ae> %ad%ncommitter %cn <%ce> %cd%nencoding %e%n%n%s%n%n%b', hash))
          call search('^parent ')
          if getline('.') ==# 'parent '
            silent delete_
          else
  ...
  try
    ...
    elseif s:buffer.type('blob')
      ...
      elseif getline('.') =~# '^tree \x\{40\}$'
        let ref = matchstr(getline('.'), '\x\{40\}')
        if s:repo() . s:rev_parse(myhash . ':') ==# ref
          let ref = myhash . ':'
        endif
        return s:Edit(a:mode, 0, ref)
...
function! fugitive#statusline(...)
  ...
  let status = ''
  if s:buffer() . s:commit() !=# ''
    let status .= ':' . s:buffer() . s:commit()[0 : 7]
  endif
...
function! fugitive#head(...)
  ...
  return s:repo() . s:head(a:0 ? a:1 : 0)
endfunction
